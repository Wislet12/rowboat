...
export const LiveNoteSchema = z.object({
    ...
    effort: z.enum(['low', 'medium', 'high', 'xhigh', 'max', 'ultra']).optional().describe('ADVANCED — leave unset. Reasoning effort paired with the per-note model override; unset means Auto (provider default). Only meaningful alongside `model`.'),
