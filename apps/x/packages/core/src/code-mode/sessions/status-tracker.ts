...
export class CodeSessionStatusTracker {
    ...
    private async handle(event: TurnBusEvent): Promise<void> {
        ...
        // Notification setup may lazily load platform services. It is a
        // best-effort side effect and must never stall the authoritative
        // ordered status projection (a slow notification previously left
        // later needs-you/idle events queued for seconds under load).
        void this.notify(sessionId, previous, next).catch(() => {});
