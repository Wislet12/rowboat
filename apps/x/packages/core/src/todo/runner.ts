...
async function driveTurn(
    ...
    message: string,
    subUseCase: string,
    modelOverride?: { provider: string; model: string; effort?: 'low' | 'medium' | 'high' | 'xhigh' | 'max' | 'ultra' },
    autoPermission = true,
): Promise<TodoRunResult> {
    ...
    try {
        ...
        try {
            ...
            const selection: { provider: string; model: string; effort?: 'low' | 'medium' | 'high' | 'xhigh' | 'max' | 'ultra' } =
...
export async function runTodoItem(
    ...
    context?: string,
    opts?: {
        model?: { provider: string; model: string; effort?: 'low' | 'medium' | 'high' | 'xhigh' | 'max' | 'ultra' };
...
export async function replyHomeChat(
    sessionId: string,
    message: string,
    opts?: { model?: { provider: string; model: string; effort?: 'low' | 'medium' | 'high' | 'xhigh' | 'max' | 'ultra' }; autoPermission?: boolean },
): Promise<HomeChatResult> {
